...
from utils import RegistryParse
...
class RegistryValue(object):
    ...
    def __repr__(self):
        return 'RegistryValue(name="{0}", value="{1}", type="{2}")'.format(self.name(), self.value(),
                                                                           self.value_type_str())

    def JSON(self):
        data = {
            "name": self.name(),
            "type": self.value_type_str(),

        }
        val = self.value()
        if type(val) == bytes:
            data["b64Value"] = base64.b64encode(self.value()).decode('utf-8')
        else:
            data['value'] = val
